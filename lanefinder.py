...
def findlanes(binary_warped, visualize=False):
    hs = binary_warped.shape[0]//2
    histogram = np.sum(binary_warped[hs:,:], axis=0)
...
def fitlanes(binary_warped, left_fit, right_fit, visualize=False):
    ...
    if not visualize:
        return ploty, current_left_fit, current_right_fit, left_fitx, right_fitx
    ...
    return ploty, current_left_fit, current_right_fit, result
